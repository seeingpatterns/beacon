...
// 문자는 항상 한/영 병기 — 받는 가족은 한국어, 구조대·현지인이 볼 땐 영어가 생명이다.
export function sosText({ km, lat, lon, t }) {
  const latS = lat.toFixed(5).replace(/0+$/, '').replace(/\.$/, '');
  const lonS = lon.toFixed(5).replace(/0+$/, '').replace(/\.$/, '');
  const lines = [
    '🆘 BEACON 위치 공유 / Location share — cyclist crossing Australia',
    `시각/Time: ${t}`,
  ];
  if (km != null) lines.push(`경로/Route: Eyre Hwy ${Math.round(km)}km 지점 (${Math.round(km)} km from Melbourne)`);
  lines.push(`좌표/GPS: ${latS}, ${lonS}`);
  lines.push(`지도/Map: https://maps.google.com/?q=${latS},${lonS}`);
  return lines.join('\n');
}
